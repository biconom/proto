...
pub struct Image {
    ...
    pub variants: ::std::collections::HashMap<
...
pub mod image {
    ...
    pub mod id {
        ...
        pub enum Identifier {
            ...
            #[prost(string, tag = "1")]
            FileId(::prost::alloc::string::String),
        }
    }
...
pub mod transaction {
    ...
    pub mod group {
        ...
        pub mod entry {
            ...
            /// Метаданные карточки получения «скользящего» матчинг-бонуса (сплит бонуса).
            ///
            /// Контекст: матчинг-бонус — это автоматическое распределение части
            /// дохода нижестоящего партнёра-инициатора (например, 50% от его клейма
            /// дивидендов) вверх по реферальной цепочке. Каждый спонсор получает
            /// долю по «скользящей» формуле:
            /// target = min((sponsor_deposit × full_bonus) / initiator_deposit, full_bonus)
            /// payout = max(target − уже_выплачено_ниже, 0)
            /// Эта карточка отображает ОДНУ такую выплату конкретному получателю.
            ///
            /// Все денежные суммы — отформатированные строки в валюте проводки
            /// (значение `currency_id` транзакции, не обязательно USDT — система
            /// готова к будущим валютам).
            #[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
            pub struct SplitMatchingBonusDetails {
                /// Идентификатор дистрибьютора-инициатора, чьё действие
                /// (например, claim дивидендов) спровоцировало распределение.
                /// Используется UI для отображения «бонус от \<Имя>».
                #[prost(uint32, tag = "1")]
                pub initiator_distributor_id: u32,
                /// Глубина данного получателя в цепочке от инициатора.
                /// 1 = прямой реферер инициатора, 2 = реферер реферера, и т.д.
                #[prost(uint32, tag = "2")]
                pub depth_level: u32,
                /// Полный объём матчинг-бонуса в группе, который распределялся
                /// вверх по структуре (РБ в терминах ТЗ). Сумма, что текущий
                /// пользователь получил, лежит в `amount` самой проводки;
                /// здесь — «общий пирог», от которого считались доли.
                #[prost(string, tag = "3")]
                pub original_matching_amount: ::prost::alloc::string::String,
                /// Снимок депозита (суммарных инвестиций пользователя в WIN-токен)
                /// в момент выплаты. Используется в формуле расчёта доли как
                /// числитель: чем больше депозит — тем больше доля. Сохраняется
                /// на момент проводки, последующие изменения депозита его не меняют.
                #[prost(string, tag = "4")]
                pub sponsor_deposit_at_pay: ::prost::alloc::string::String,
                /// Теоретический максимум, который данный пользователь мог
                /// получить с этой группы матчинг-бонуса по своему депозиту
                /// (значение `target` из формулы выше, ДО вычета «уже выплачено ниже»).
                /// Полезно для UI «вы получили X из возможных Y».
                #[prost(string, tag = "5")]
                pub target_limit: ::prost::alloc::string::String,
                /// Сумма, которая прошла мимо данного пользователя и ушла дальше
                /// вверх по цепочке (либо вернулась в org-пул, если цепочка кончилась).
                /// Это «упущенная выгода» — сколько ещё можно было бы получить,
                /// будь депозит больше или окажись выше в структуре.
                #[prost(string, tag = "6")]
                pub missed_amount: ::prost::alloc::string::String,
                /// Был ли активен у пользователя флаг полного наследования
                /// матчинг-бонуса (`Flags::MATCHING_BONUS_FULL_INHERITANCE`).
                /// Если `true` — пользователь забрал весь остаток группы независимо
            ...
            pub struct StakingReferralDetails {
                ...
                /// НЕДОЗАРАБОТОК: сколько партнёр заработал бы с этого начисления
                /// на максимальном ранге (16%), сверх того, что заработал. Ноль —
                /// взял всё возможное. Величина справочная, этих денег не
                /// существует: это показатель «сколько даст рост ранга».
                ///
                /// У «токена упущенной выгоды» (начисление в одну минимальную
                /// единицу валюты сквозному предку, которому по лестнице не
                /// причиталось ничего) `rate` равен нулю, а здесь стоит весь
                /// упущенный потолок — ради этой цифры токен и начисляется.
                #[prost(string, tag = "5")]
...
pub mod confirmation {
    ...
    pub mod field {
        ...
        pub struct Chanel {
            ...
            pub chanel_kind: i32,
            /// Безопасный идентификатор канала для отображения:
            #[prost(string, tag = "2")]
            pub chanel_contact: ::prost::alloc::string::String,
            /// EMAIL   → замаскированный email (например, "u\*\*\*@gmail.com")
            /// TELEGRAM → username бота (например, "@MyProjectBot")
            ///
        ...
        pub mod chanel {
            ...
            pub enum Kind {
                Unspecified = 0,
                /// Код отправляется на email-адрес пользователя.
                Email = 1,
                /// Код отправляется через Telegram-бота.
                /// chanel_contact содержит username бота (например, "@MyProjectBot").
                /// Фактический Telegram-аккаунт получателя на клиент НЕ передаётся.
                Telegram = 2,
            }
            impl Kind {
                /// String value of the enum field names used in the ProtoBuf definition.
...
pub mod staking {
    ...
    pub struct Deposit {
        ...
        pub updated_at: ::core::option::Option<::prost_types::Timestamp>,
        /// ── Маркетинг ──
        /// true — депозит вне маркетинга: прирост его тела НЕ раздаётся вверх
        /// по реферальной лестнице и НЕ входит в объёмы для рангов (личный
        /// вклад, объём ветки, командный объём) — ни при создании, ни при
        /// реинвесте прибыли. Доходность самому партнёру и тир считаются как
        /// обычно. Депозит, открытый телом этого (`ReinvestDeposit`), наследует
        /// флаг. Меняется админом (`SetDepositMarketingBlock`) в любом статусе
        /// депозита. Уже выплаченные доли лестницы не отзываются, а объёмы
        /// пересчитываются сразу: вклад депозита (его тело) снимается с объёмов
        /// при блокировке и возвращается при снятии. Ранг никогда не понижается:
    ...
    pub struct DepositsSummary {
        ...
        /// Текущая ставка тира за цикл — коэффициент строкой, всегда непустой.
        ///
        /// `"0.00"` означает, что начислять нечего: либо активных депозитов нет
        /// (`active_count == 0`), либо их суммы не хватает даже на нижнюю ступень
        /// сетки. Второй случай — реальная нулевая ставка, поэтому отличать его
        /// пустой строкой было бы неверно. Показывать ли блок ставки, решается по
        /// `active_count`, а не по этому полю.
        #[prost(string, tag = "4")]
    ...
    pub mod event {
        ...
        pub struct ReferralReceived {
            ...
            #[prost(uint32, tag = "6")]
            pub depth: u32,
            /// НЕДОЗАРАБОТОК: сколько партнёр ЗАРАБОТАЛ БЫ с этого же начисления,
            /// будь он на максимальном ранге (16%), сверх того, что заработал.
            ///
            /// Считается от той же базы и от того же, что уже причиталось
            /// нижестоящим звеньям, поэтому это потолок КОНКРЕТНО ЭТОГО
            /// начисления, а не абстрактные «16% от базы». Ноль означает «взял
            /// всё возможное»: либо максимальный ранг, либо нижестоящие уже
            /// выбрали лимит.
            ///
            /// Вычитается заработанное ЛЕСТНИЦЕЙ, а не всё зачисленное. Разница
            /// видна на «токене упущенной выгоды»: сквозной предок, которому не
            /// причиталось ничего, получает `amount` в одну минимальную единицу
            /// валюты, а `lost_profit` показывает весь упущенный потолок целиком.
            /// Поэтому `amount + lost_profit` потолку больше не равно — оно
            /// превышает его ровно на этот токен.
            ///
...
pub mod license {
    ...
    pub mod voucher {
        ...
        pub struct List {
            #[prost(message, repeated, tag = "1")]
            pub items: ::prost::alloc::vec::Vec<super::Voucher>,
        }
    }
    ...
    pub struct State {
        ...
        pub status: i32,
        /// Идентификатор текущего активного ваучера
        #[prost(uint32, tag = "2")]
        pub current_voucher_id: u32,
        /// Список ваучеров, связанных с лицензией
        #[prost(message, repeated, tag = "3")]
    ...
    pub mod state {
        ...
        pub enum Status {
            Unspecified = 0,
            /// Начальное состояние (лицензия никогда не активировалась)
            Inactive = 1,
            /// Активная
            Active = 2,
...
pub mod subscription {
    ...
    pub mod state {
        ...
        pub enum Status {
            Unspecified = 0,
            /// Начальное состояние (подписка никогда не приобреталась)
            Inactive = 1,
            /// Оплачена и активна
            Active = 2,
            /// Оплата не прошла, ожидается повтор
            PastDue = 3,
            /// Подписка завершена или отменена
            Canceled = 4,
...
pub mod marketing_slot {
    ...
    pub struct DistributorState {
        ...
        /// Имеет ли текущий авторизованный пользователь право на просмотр профиля данного дистрибьютора
        #[prost(bool, tag = "3")]
        pub viewable: bool,
    }
...
pub mod win_time {
    ...
    #[derive(Clone, Copy, PartialEq, Eq, Hash, ::prost::Message)]
    pub struct Balance {
        /// Текущий баланс токенов (знаковый)
        #[prost(int64, tag = "1")]
        pub amount: i64,
    }
}
...
pub mod geo {
    ...
    pub struct Cluster {
        ...
        ///
        /// Широта центра ячейки (градусы)
        #[prost(double, tag = "1")]
        pub latitude: f64,
        /// Долгота центра ячейки (градусы)
        #[prost(double, tag = "2")]
        pub longitude: f64,
        /// Всего дистрибьюторов в кластере
        #[prost(uint32, tag = "3")]
        pub distributors_count: u32,
        /// Суммарный баланс WinTime кластера (знаковый)
        #[prost(int64, tag = "4")]
        pub win_time_balance: i64,
        ...
        #[prost(message, repeated, tag = "5")]
        pub top: ::prost::alloc::vec::Vec<Pin>,
    }
}
