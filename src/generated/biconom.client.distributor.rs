...
pub struct GetBreadcrumbsResponse {
    ...
    #[prost(uint32, tag = "2")]
    pub view_distributor_id: u32,
    /// Цепочка сверху вниз, без исключений: `\[0\]` — всегда `executor_distributor_id`,
    /// последний — всегда `view_distributor_id`, между ними промежуточные звенья.
    /// Если запрошен сам себя — ровно один элемент. Вышестоящих запрашивающего в
    /// цепочке не бывает никогда: путь ведёт только вниз.
