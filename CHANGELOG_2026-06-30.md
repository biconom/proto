...
message WinTime {
    ...
    message Balance {
        ...
        uint32 group_seq = 3;      // последний group_seq владельца (курсор для ListTransactionGroups)
        repeated TypeStat stats = 4; // НОВОЕ: статистика по типам за всё время (см. ниже)
    }
}
```
...
>
> `Balance.stats` (поле 4) — суммарная статистика по типам за всю историю владельца
> (по записи на тип с `count > 0`, `sum` знаковый). Агрегат, **не зависит** от
> фильтра/пагинации. Теперь живёт **внутри баланса**, а не отдельным полем ответа —
> поэтому едет в любом ответе с балансом, включая **`WalletCurrencyService/List`**
> (поле `win_time`).

### `biconom/client/win_time/win_time.proto`

**(1) Статистика** — `repeated TypeStat stats` живёт **внутри `Balance`** (`Balance.stats`,
поле 4 в `types/win_time.proto`), а не отдельным полем ответа. Поэтому отдаётся в любом
ответе с балансом: `GetBalance`, `ListTransactions`, `ListTransactionGroups` **и
`WalletCurrencyService/List`** (поле `win_time`). Агрегат за всю историю, не зависит от
фильтра/пагинации. Возвращаются только типы с `count > 0`.

> ⚠️ Перенос: ранее `stats` было отдельным полем `BalanceResponse` (6) и
> `TransactionGroupsResponse` (4). Теперь оба этих поля `reserved`, читать `stats`
> нужно из `balance.stats`.
...
```javascript
// Статистика по типам — теперь ВНУТРИ balance (в любом ответе с балансом).
const bal = await WinTimeServiceClient.GetBalance({});
for (const s of bal.balance.stats) {
    renderTypeStat(s.type, s.count, s.sum); // sum знаковый
}

// Та же статистика доступна в WalletCurrencyService/List через поле win_time:
const list = await WalletCurrencyServiceClient.List({ /* ... */ });
for (const s of list.winTime?.stats ?? []) {
    renderTypeStat(s.type, s.count, s.sum);
}
...
| Статистика по типам `repeated TypeStat stats` — теперь **внутри** `Balance.stats` | `types/win_time.proto` (field 4); едет и в `WalletCurrencyService/List.win_time` |
