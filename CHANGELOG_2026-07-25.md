...
> **Напоминание (не новое поле, но теперь актуально).** С появлением входа через
> Telegram у аккаунта два равноправных варианта идентичности: **email + пароль**
> или **Telegram** (у Telegram-пользователей email может отсутствовать —
> `User.telegram_username = 8` заполнен вместо него). Это давно отражено в форме
> подтверждения: `Confirmation.Method` содержит вариант **`TELEGRAM = 2`** — код
> подтверждения доставляется через Telegram-бота, а в поле `address` приходит
> **username бота** (например `@MyProjectBot`), сам Telegram-аккаунт получателя
> на клиент не передаётся. Фронту следует рендерить ОБА способа доставки кода
> (email / Telegram), а не предполагать email по умолчанию.
