...
Имя графика (`chart_name`) сравнивается без учёта регистра и с обрезкой пробелов. Неизвестное имя в `GetChart` — ошибка `NOT_FOUND`; в пакетном `GetCharts` такой элемент просто пропускается.

| `chart_name` | Тип | Что показывает |
|---|---|---|
| `trade_win_usdt` | Монетарный | **Покупка** токена за USDT по паре `WIN-USDT`. Считается только направление «отдали котируемую валюту — получили базовую»: при `currency_id` = USDT это потраченные USDT, при `currency_id` = WINZU — полученные токены. Продажа токена в график не входит. Комиссии сделки не учитываются. |
| `income` | Монетарный | Все бонусные начисления на кошелёк партнёра — маркетинг, матчинг, дивидендный пул, арена, квесты, WinTime, стейкинг. Показывает **начисленное**: доход стейкинга попадает сюда и при включённом реинвесте прибыли. |
| `registrations` | Количественный | Регистрации. В режиме `PERSONAL` — лично приглашённые партнёры (первая линия), в `TEAM` — приглашённые каждым участником структуры. |
| `license_purchases` | Монетарный | Покупки лицензий партнёрами. |
| `staking_deposits` | Монетарный | **Валовой** приток денег в стейкинг: сколько ушло с кошельков в тела депозитов. Включает открытие депозита, реинвест тела созревшего депозита и реинвест прибыли цикла. Реинвест — это повторное вложение уже учтённых денег, поэтому график показывает оборот программы, а не приток новых средств. |
...
Отдельного режима «первая линия» нет и не требуется: это `TEAM` с `team_depth_limit: 1` и `include_personal: false`.
...
- **Назначение**: Получить суммарную финансовую статистику по всем валютам и платежным сетям.
- **Использование**: Этот метод вызывается без параметров и предназначен для получения общих агрегированных данных. Полезен для административных панелей для мониторинга состояния системы.
- **Запрос**: Принимает `google.protobuf.Empty`, что означает отсутствие передаваемых параметров в теле запроса.
- **Ответ**: Возвращает объект `SummaryStatisticsResponse`, который содержит детализированную статистику, сгруппированную по валютам.
...
## 5. Структуры данных для GetSummary

### `SummaryStatisticsResponse`
Основной контейнер для ответа на запрос `GetSummary`.

- `repeated CurrencySummary summaries`: Список статистических данных, где каждый элемент представляет одну валюту.

### `CurrencySummary`
Агрегированная статистика для конкретной валюты.

- `uint32 currency_id`: Идентификатор валюты.
- `string total_user_balance`: Общая сумма средств на счетах всех пользователей в данной валюте.
- `repeated PaymentNetworkSummary payment_network_stats`: Список статистических данных по каждой платежной сети, работающей с этой валютой.

### `PaymentNetworkSummary`
Детализированная статистика для конкретной платежной сети в рамках одной валюты.

- `uint32 payment_network_id`: Идентификатор платежной сети.
- `string total_deposit`: Общая сумма пополнений.
- `string total_deposit_fee`: Общая сумма комиссий за пополнение.
- `string total_withdrawal`: Общая сумма выводов.
- `string total_withdrawal_fee`: Общая сумма комиссий за вывод.
- `optional string hot_wallet_balance`: Текущий баланс на горячем кошельке данной сети (если применимо).