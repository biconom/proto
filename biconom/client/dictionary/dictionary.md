...
- **Ответ**: Возвращает `PublicResponse`, который содержит списки всех основных справочных моделей (`Currency`, `Exchange`, `PaymentNetwork`, `Tree`, `License.Plan` и т.д.), а также агрегатные показатели системы: `marketing_v2` и `distributors_count`.
- **Авторизация**: Не требуется.

#### Поле `distributors_count`
Суммарное количество дистрибьюторов в системе. Считается как последний выданный идентификатор дистрибьютора (high-water-mark последовательности), то есть общее число когда-либо созданных дистрибьюторов. Значение монотонно не убывает и не уменьшается при деактивации отдельных дистрибьюторов.
