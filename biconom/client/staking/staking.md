...
**Идемпотентность.** `CreateDeposit` принимает `idempotency_key`: создание депозита —
денежная операция с реферальной раздачей вверх по дереву, и повтор запроса (двойной
тап, ретрай сети) без ключа создал бы **второй** депозит и вторую раздачу. С ключом
повтор возвращает уже созданный депозит.
