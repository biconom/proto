...
| `GetRankProgress` | Детальный разбор ранга с разбивкой по веткам (`legs`). По умолчанию свой, по `Distributor.Id` — партнёра из своей команды |
...
| `current_rate` | Текущая ставка за цикл, коэффициентом (`"0.07"`). Всегда непустая; `"0.00"` = начислять нечего. Блок ставки прятать по `active_count == 0`, а не по этому полю |
...
Запрос метода — `Distributor.Id` напрямую, без обёртки: пустой идентификатор
означает свой прогресс, заданный (id или username) — прогресс партнёра из своей
команды. Право проверяется тем же гейтом, что у
`DistributorService.Get` (свой / команда / слотовая иерархия, плюс глобальный
просмотр у поддержки); нет права — `PermissionDenied`, нет такого — `NotFound`.
В ответе поле `distributor_id` называет, ЧЕЙ прогресс показан, а `summary` несёт
его цифры по депозитам — внесено за всё время, активно сейчас, начислено дохода,
созрело и ждёт решения. Для чужой карточки это единственный источник: `GetState`
всегда про себя.
...
`Leg.current_rank` — текущий ранг лидера ветки (0 — квалификации нет), под
строку «N Rank · X %» в карточке ветки. Процент и требования ранга берутся из
`GetConfig → ranks[]` по номеру, как для шапки экрана.
