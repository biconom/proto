...
- **`PurchaseLicensePlan`**: Приобретает тарифный план лицензии. При повторной покупке создаются дополнительные ваучеры, продлевающие период действия лицензии. Возвращает обновлённое состояние слота (`MarketingSlot.State`).
- **`ListPendingManualPlacementSlots`**: Возвращает список слотов («кандидатов»), ожидающих ручной расстановки. Опционально принимает `distributor_id` — если не указан, используется авторизованный пользователь. Ответ обогащён данными о владельцах для отображения карточек.
