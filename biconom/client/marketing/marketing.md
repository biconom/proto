...
- **`GetSlotBreadcrumbs`**: Возвращает «хлебные крошки» по слотам — путь по маркетинговому дереву от активного слота запрашивающего вниз до указанного слота, со всеми промежуточными звеньями и справочниками для отрисовки (`slots` → `distributors` → `accounts`). Slot-контекст сессии не используется: дерево берётся у запрошенного слота, а слот запрашивающего ищется уже в этом дереве. Путь ведёт только вниз — для вышестоящего слота, соседней ветки или чужого дерева возвращается `SLOT_VIEW_PERMISSION_DENIED`. В отличие от `GetSlot`, где `breadcrumbs` — это цепочки по `MARKETING_LEVEL_LIMIT` с номерами веток, здесь отдаётся плоский линейный список.
