...
- **`CalculateManualPlacementPrice`**: Универсальный метод для предпросмотра и расчета стоимости расстановки.
  ...
  В обоих случаях ответ содержит контекст (дерево) целевого места для визуализации и `default_price` — базовую цену за ручную расстановку для данного дерева. Использует `ManualPlacementRequest`.
