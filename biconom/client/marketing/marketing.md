...
- **`PurchaseCapacityUpgrade`**: Осуществляет покупку дополнительных мест в первой линии. Слот определяется автоматически текущим авторизованным пользователем. Возвращает обновленный объект `biconom.types.Slot`.

### Расстановка слотов
- **`ListPendingManualPlacementSlots`**: Возвращает список слотов ("кандидатов"), которые находятся в буфере и ожидают ручной расстановки. Ответ содержит время автоматического сброса (если применимо) и обогащен данными о владельцах (дистрибьюторах/аккаунтах) для отображения карточек.
