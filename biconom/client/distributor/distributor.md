...
### `rpc ChangeUsername(ChangeUsernameRequest) returns (Response)`
- **Назначение**: Изменить `username` дистрибьютора для авторизованного пользователя. Идентификатор дистрибьютора вычисляется автоматически на основе текущей сессии.
- **Использование**: Позволяет пользователю обновить свой псевдоним/бренд. Метод вернет обновленную модель дистрибьютора в рамках стандартного `Response`.
- **Параметры `ChangeUsernameRequest`**:
  - `string new_username`: Желаемое новое имя пользователя.

### `rpc GetBreadcrumbs(biconom.types.Distributor.Id) returns (GetBreadcrumbsResponse)`
- **Назначение**: Получить «хлебные крошки» — путь по реферальному дереву от авторизованного дистрибьютора вниз до указанного, со всеми промежуточными звеньями.
- **Использование**: Навигационная строка вида «Я / Партнёр А / Партнёр Б / Целевой» при провале вглубь структуры. Ответ сразу содержит справочники дистрибьюторов и аккаунтов (с аватарами и контактами), поэтому дополнительных запросов на отрисовку не требуется.
- **Параметры**: принимает `biconom.types.Distributor.Id` напрямую — тот же идентификатор, что и `Get` (глобальный `id`, `username` или составной ключ). Если идентификатор не заполнен, целевым считается сам авторизованный дистрибьютор, и цепочка состоит из одного звена.
- **Доступ**: метод отдаёт цепочку, только если целевой дистрибьютор — это сам авторизованный (`Personal`) или его потомок в реферальном дереве (`Team`). Для вышестоящего, соседней ветки или чужой сети возвращается `PERMISSION_DENIED`: крошек относительно запрашивающего не существует. Глобальное право просмотра (админ/саппорт) на это не влияет — оно даёт видеть чужой профиль, но не создаёт пути к нему; вышестоящие запрашивающего в цепочку не попадают никогда.
...
- `slot_states`: Список состояний маркетинговых слотов данного дистрибьютора.
- `dividend_pool_bonus`: Ожидающий бонус дивидендного пула. Заполняется только при просмотре собственного профиля (executor == view). Для чужих профилей — отсутствует.
...
- `slot_states`: Список состояний маркетинговых слотов для всех возвращенных дистрибьюторов.
...
Ответ с цепочкой навигации, построенный по тем же правилам плоских дедуплицированных справочников:
- `executor_distributor_id`: ID дистрибьютора, выполняющего запрос.
- `view_distributor_id`: ID запрошенного дистрибьютора — хвост цепочки.
- `chain_ids`: цепочка сверху вниз — `[0]` всегда `executor_distributor_id`, последний элемент — `view_distributor_id`. При запросе самого себя содержит ровно один элемент.
- `distributors`: дедуплицированные дистрибьюторы всех звеньев цепочки (с `relationship_state` относительно запрашивающего).
- `accounts`: дедуплицированные аккаунты-владельцы звеньев; внутри аккаунта — `User` с аватаром и контактами (`email`, `telegram_username`).
