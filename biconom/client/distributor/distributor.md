...
- **Доступ**: метод отдаёт цепочку, только если целевой дистрибьютор — это сам авторизованный (`Personal`) или его потомок в реферальном дереве (`Team`). Для вышестоящего, соседней ветки или чужой сети возвращается `PERMISSION_DENIED`: крошек относительно запрашивающего не существует. Обладатели глобального права просмотра (админ/саппорт) получают полную вышестоящую линию целевого до корня сети.
