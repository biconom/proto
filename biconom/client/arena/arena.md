...
**Арена** — постоянный соревновательный контейнер. В системе 5 арен:
...
| 5 | `staking` | Прирост тела депозита стейкинга — вкладчику и его прямому спонсору (значение = сумма в USDT, `value_precision = 6`) |

Арена 5 зарегистрирована заранее и до запуска цикла отдаёт `cycle = null`,
`total_cycles = 0` — состояние «циклов ещё не было» из контракта `Arena.Card`.
