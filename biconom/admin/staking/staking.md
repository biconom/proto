...
| RPC | Назначение | Право |
|---|---|---|
| `GrantDeposit` | Подарить партнёру депозит за счёт компании. Две группы проводок: подарок через кассу на кошелёк (карточка `gift`), затем покупка депозита с кошелька. Минимум, `BLOCK_STAKING` и `PAUSED` не проверяются | `ROOT` |
...
`marketing_blocked` выводит депозит из маркетинга: прирост его тела не идёт по
реферальной лестнице и не входит в объёмы для рангов (создание и реинвест
прибыли). Доходность партнёру и тир считаются как обычно. Депозит, открытый телом заблокированного
...
Переключение флага пересчитывает объёмы сразу: вклад депозита (его тело)
...
(весь оборот команды), `team_volume` = $22 000 (засчитано) — одна ветка дала
$55 000, но лимит на ветку для ранга 5 ($17 000 — это 34% требования) срезал её до
лимита, остальные $5 000 засчитаны целиком. Разбивка `legs` показывает, какая
именно ветка обрезана (`capped == true`).

Вместе с `legs` приходят справочники `distributors` и `accounts` — карточки лидеров
веток (логин, аватар, контакты), чтобы поддержка видела, кто именно упёрся в лимит,
без второго запроса. Сопоставление по id:
`Leg.distributor_id → Distributor.id → Distributor.account_id → Account.id`.
В `distributors` приходит и САМ запрошенный дистрибьютор — поддержка спрашивает по
username и иначе не увидела бы ни аватара, ни идентификатора.

Сколько партнёров структуры реально участвуют в стейкинге, показывают
`stakers_partners` (личники) и `stakers_team` (вся команда, без самого партнёра).
По каждой ветке тот же разрез: `Leg.personal_activated`, `Leg.stakers_partners`,
`Leg.stakers_team`. Считаются партнёры, а не депозиты, и счёт пожизненный —
закрытый депозит партнёра из него не убирает. `Leg.current_rank` — текущий ранг
лидера ветки (0 — нет квалификации); процент — из `GetConfig → ranks[]`.
