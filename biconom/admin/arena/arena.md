...
| 5 | `staking` | Прирост тела депозита стейкинга — вкладчику и прямому спонсору (=сумма в USDT). Зарегистрирована без цикла: запуск — `StartCycle` |
...
```protobuf
oneof identifier {
    uint32 id   = 1;  // числовой: 1, 2, 3, 4, 5
    string name = 2;  // строковый: "wincoin", "win_lite", "win_pro", "win_ultra", "staking"
}
```
