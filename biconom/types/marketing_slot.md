...
- **`tree_id`**: Идентификатор дерева, в котором находятся слоты. И `executor_slot_id`, и `view_slot_id` принадлежат этому дереву.
...
- **`slot_states`**: Список динамических состояний (`State`). Каждый объект содержит:
  - `tree_id` — идентификатор дерева, к которому относится слот.
  - `license_state` / `subscription_state` — техническое право доступа и состояние биллинга.
  - `placement_required` — флаг, указывающий, что слот ещё не расставлен в иерархии и нуждается в размещении.
  - `placement_deadline_at` — дедлайн ручной расстановки. После этого момента ручная расстановка запрещена и сработает автоматическая.
  - `placement_executed_at` — фактическое время расстановки слота (заполняется после размещения).
...
---
*Документация актуальна для версии proto-файлов от 14 февраля 2026 г.*