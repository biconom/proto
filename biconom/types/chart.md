...
- **`custom_id`**: Опциональный строковый идентификатор, который возвращается, если был указан в запросе `GetChartRequest`. Позволяет клиенту надежно сопоставить ответ с конкретным запросом, даже если `chart_name` одинаковый.
